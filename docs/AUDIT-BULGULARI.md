...
- ☑ **[AppFoundation/actor-reentrancy-toctou]** Packages/AppFoundation/Sources/AppFoundation/Push/DeviceTokenRegistering.swift:51 — DÜZELTİLDİ (task-chain serialize: load→POST→save atomik; AppFoundation 288 test yeşil)
...
- ☑ **[PlayerKit/resume-position-lost]** Packages/PlayerKit/Sources/PlayerKit/Pool/PlayerPool.swift:342 — DÜZELTİLDİ (resumeSeek: .loading'de pendingResumePosition'a ertele; PlayerKit 239 test yeşil)
...
- ☑ **[ProfileKit/reset-partial]** Packages/ProfileKit/Sources/ProfileKit/BildirimMerkezi/NotificationCenterModel.swift:114 — DÜZELTİLDİ (tombstone clamp yalnız tam-liste/nextCursor==nil load'da; ProfileKit 161 test yeşil)
...
- ☑ **[PlayerKit/playback-auth-recovery]** Packages/PlayerKit/Sources/PlayerKit/Engine/PlaybackEngine.swift:265 — DÜZELTİLDİ (recoveryAttempts firstFrameReady'de sıfırlanır → bağımsız re-expiry yine kurtarılır; ardışık reload-fail firstFrame'e ulaşmaz → loop-guard korunur; 04 §6.4/T11 uyumlu; RED→GREEN + repurposed loop-guard testi, PlayerKit 240 test yeşil)
...
- ☑ **[PlayerKit/stall-recovery]** Packages/PlayerKit/Sources/PlayerKit/Engine/AVPlayerBackend.swift:206 — DÜZELTİLDİ (stall mantığı saf StallTracker value-type'ına ayrıldı + pause() reset eder → pause/resume arası bayrak takılı kalmaz; StallTracker 7 unit test yeşil, PlayerKit 248 test; KVO/notif wiring compile+manuel App doğrulaması)
...
- ⊘ **[ProfileKit/linking-merge-desync]** Packages/ProfileKit/Sources/ProfileKit/Profil/ProfilModel.swift:108 — KABUL (gerekçe): hesap-değişimi↔cüzdan koordinasyonu doğru KATMANDA (App), ince VM'de DEĞİL (R2/R8). switch (FARKLI userID) App'te WalletStore.reset + refetchForNewAccount tetikler → observeWallet yeni cüzdanı alır (AccountSwitchDataFlowTests kanıt). link (misafir→bağlı, AYNI userID) sunucu-otoriter bakiyeyi KORUR → refetch gerekmez. session-death observeWallet guard'ıyla otoriter temizlenir (aşağıdaki sticky-clear fix). ProfilModel'in iki akışı bağımsız gözlemesi bilinçli tasarım.
...
- ☑ **[AppFoundation/torn-token-write]** Packages/AppFoundation/Sources/AppFoundation/Session/TokenRefreshCoordinator.swift:117 — DÜZELTİLDİ (refresh ÖNCE yazılır + access best-effort → torn çift self-heal; 2 RED→GREEN test, AppFoundation 291 test yeşil)
...
- ⊘ **[ProfileKit/reset-race]** Packages/ProfileKit/Sources/ProfileKit/Profil/ProfilModel.swift:83 — KABUL (gerekçe): load() cache-first best-effort snapshot'tır; observeUpdates() load()'ı ÖNCE çalıştırıp ARDINDAN canlı akışları başlatır ve akış replay'i (her zaman ≥ snapshot tazeliğinde) SON söz sahibidir (review #12, liveStreamValueWinsOverStaleInitialLoadSnapshot testi). İki await arası olası çapraz-hesap tutarsızlığı GEÇİCİdir ve akış replay'iyle anında düzeltilir. Atomik birleşik snapshot portu olmadan load()'ı tek-atım tutmak mimari gereksiz karmaşa; akış-otoritesi doğru çözüm.

- ☑ **[ProfileKit/wallet-clear-stickiness]** Packages/ProfileKit/Sources/ProfileKit/Profil/ProfilModel.swift:102 — DÜZELTİLDİ (CI flaky `sessionExpiredClearsStaleWalletDisplay` kökü + gerçek gap): observeWallet, sessionExpired'de geç/replay cüzdan emission'ını yok sayar → temizlik OTORİTER; observeWallet↔observeSession sıra yarışı deterministik. RED→GREEN sticky-clear testi, ProfilModelTests 3× deterministik yeşil.
