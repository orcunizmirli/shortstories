...
**Kabul kriterleri:**
- [x] İstem yalnız pozitif an tetiklerinde çağrılır: check-in streak günü + **bölüm-tamamlama** (playedToEnd → auto-advance kararı, render-loop DIŞI; `PlayerFeedDelegate.playerFeedDidCompleteEpisode` → HomeCoordinator → `requestReviewIfEnabled(.episodeCompleted)`); oynatma/paywall/hata ekranında ASLA çağrılmaz.
...
**Etki notu:** Pozitif tetikler (check-in + bölüm-tamamlama) + negatif sinyal (satın-alma-hatası) BAĞLI. Kalan tek KAYNAK: oynatma-hatası negatif sinyali (yeni `didFailPlayback` delegate event'i gerektirir; düşük artımlı — negatif-sinyal mekanizması + satın-alma-hatası zaten kapsıyor) → takibe yazıldı. Mekanizma (policy+controller+kill-switch+analitik+negatif-sinyal) TAM + test-edilebilir.
