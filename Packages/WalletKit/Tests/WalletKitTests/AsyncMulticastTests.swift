...
struct AsyncMulticastTests {
    ...
    @Test func concurrentSendNeverLeavesStaleSeedLast() async {
        // Deterministik regresyon (adversarial self-review güçlendirmesi; ProfileKit AsyncMulticast ile aynı seam):
        // seed kilit DIŞINDA yield edilirse, kayıt-sonrası pencerede araya giren send(1) aboneyi [1, 0] sırasıyla
        // besler → abonenin SON gördüğü değer bayat 0 kalır. Kilit-içi atomik seed ile SON değer en-yeni (1) olmalı.
        let multicast = AsyncMulticast<Int>()
        multicast.send(0) // latest = 0
        multicast.onRegisteredForTesting = { multicast.send(1) } // kayıt biter bitmez araya daha yeni değer sok
