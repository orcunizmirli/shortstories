...
struct DirectorAutoAdvanceTests {
    ...
    func playedToEndAdvances() async {
        ...
        let arrived = await eventually { box.decisions == [.advance(toIndex: 1)] }
        #expect(arrived)
    }
    ...
    func lastItemRequestsMore() async {
        ...
        let arrived = await eventually { box.decisions == [.requestMoreItems] }
        #expect(arrived)
    }
    ...
    func disabledYieldsStay() async {
        ...
        let arrived = await eventually { box.decisions == [.stay] }
        #expect(arrived)
    }
}
